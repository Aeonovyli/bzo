...
export function meshFaceBlocksDirection(face, direction, obs, x, y, z) {
...
export function findMeshHitFace(obs, x, y, z, radius, height, passField = 'driveThrough', direction = null) {
  const { bounds } = obs;
...
export function findMeshHitFaceOriented(
...
export function meshIntersectsTank(obs, x, y, z, rotation, height, slack = 0, tankScale = null, direction = null) {
  const halfWidth = TANK_HALF_WIDTH * (tankScale ? tankScale.width : 1);
  const halfLength = TANK_HALF_LENGTH * (tankScale ? tankScale.length : 1);
  const trim = Math.max(0, Math.min(slack, halfWidth));
...
export function collectMeshHitFacesTank(
...
export function collectMeshHitFacesCylinder(obs, x, y, z, radius, height, passField, direction, out) {
  const { bounds } = obs;
  if (bounds && (x + radius < bounds.minX || x - radius > bounds.maxX
    || z + radius < bounds.minZ || z - radius > bounds.maxZ)) {
    return out;
...
// never is.
export function pickPriorityMeshFace(candidates, direction) {
...
// it.
export function getMeshCrossingPlane(obs, x, y, z, rotation, tankScale = null) {
  if (!obs) return null;
  const halfWidth = TANK_HALF_WIDTH * (tankScale ? tankScale.width : 1);
  const halfLength = TANK_HALF_LENGTH * (tankScale ? tankScale.length : 1);
...
export function findMeshRayImpact(obstacles, fromX, fromY, fromZ, toX, toY, toZ, radius) {
  let best = null;
  for (const obs of obstacles) {
...
export function getTankHitNormal(obs, x, y, z, rotation, toY, height, sweep = null) {
  const base = obs.baseY || 0;
