...
const THIEF_VEL_AD = 1.67;
const THIEF_TINY_FACTOR = 0.5;
const THIEF_AD_SHOT_VEL = 8.0;
const THIEF_AD_RATE = 12.0;
const THIEF_AD_LIFE = 0.05;
const THIEF_DROP_TIME_FACTOR = 0.5;
...
function getFlagGrabCount(abbreviation, maxFlagGrabs) {
...
function stealsFlags(abbreviation) {
...
function applyMotionInput(abbreviation, forward, turn, insideBuilding = false) {
  const effects = getMotionEffects(abbreviation);
...
function findNearestGroundFlag(flagIterable, x, y, z, range) {
  let closest = null;
...
// remember.
function keepFlagIdentity(incomingType, knownType, status) {
