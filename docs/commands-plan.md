...
| `/flag up`, `/flag show`, `/flag reset` | `flagMod` | `zapFlag`, `resetFlag` and the flag table are all there. `show` prints upstream's own line (`FlagInfo::getTextualInfo`) and, bzo's own half, sends the asking operator the unhidden flag states so their own map fills in -- a superflag nobody holds travels anonymous, and a client keeps every identity it is told. `reset` still takes no argument and means upstream's `reset all`; `<all\|unused\|team\|#flagId\|FlagAbbv> [noteam]` is not ported |
| `/flag take <player>`, `/flag give <player> <#flagId\|FlagAbbr> [force]` | `flagMaster` | upstream's own, whole: `take` resets the flag off a tank, `give` resets or drops whatever the tank was carrying, takes the flag off its current holder when `force` says so, and refuses a dead player. bzo's two tiers put them behind OPERATOR with the rest rather than porting the `flagMod`/`flagMaster` split |
